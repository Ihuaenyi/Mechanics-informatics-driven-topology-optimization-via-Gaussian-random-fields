"""
Karhunen-Loève Expansion Heterogeneous Specimen Generator
Modified for 2D anisotropic elasticity
"""
...
class KLExpansionSpecimenGenerator:
    ...
    def export_abaqus_script(self, filename="kl_specimen_abaqus.py"):
        ...
        # Continue with 2D part creation (adapted from 3D template)
        script_lines.extend([
            "#2D part (adapted from working 3D template)",
            "p = mdb.models['Model-1'].Part(name='Part-1', dimensionality=TWO_D_PLANAR, ",
            "    type=DEFORMABLE_BODY)",
            "p = mdb.models['Model-1'].parts['Part-1']",
            "p.BaseShell(sketch=s)",
            ...
            "a.Instance(name='Part-1-1', part=p, dependent=ON)",
            "",
            "#step - using  working step parameters",
            "mdb.models['Model-1'].StaticStep(name='Step-1', previous='Initial')",
            "mdb.models['Model-1'].steps['Step-1'].setValues(initialInc=0.05, minInc=1e-08)",
            "",
            "# AUTOMATIC EDGE MASK DETECTION FOR 2D (adapted from  3D face detection)",
            ...
            "        pass",
            "",
            "#mesh - using  working mesh parameters",
            "p = mdb.models['Model-1'].parts['Part-1']",
            ...
            "a = mdb.models['Model-1'].rootAssembly",
            "",
            "# Job - using  working job parameters",
            "mdb.Job(name='Job-1', model='Model-1', description='', type=ANALYSIS, ",
            "    atTime=None, waitMinutes=0, waitHours=0, queue=None, memory=90, ",
            "    memoryUnits=PERCENTAGE, getMemoryFromAnalysis=True, ",
            "    explicitPrecision=SINGLE, nodalOutputPrecision=SINGLE, echoPrint=OFF, ",
            ...
            "mdb.jobs['Job-1'].submit(consistencyChecking=OFF)",
            "",
            "# ------------- POST PROCESSING (adapted from  working template) ---------------------------",
            "from odbAccess import *",
            "from abaqusConstants import *",
            "import numpy as np",
            "",
...
def create_kl_specimen(eigenvalues, target_void_fraction=0.25, 
    ...
    """
    Create KL specimen with 2D anisotropic ABAQUS-compatible contours
